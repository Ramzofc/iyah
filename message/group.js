...
const bg = "https://tinyurl.com/2noqlwx4"
...
//Auto kick jika itu user yang sudah di ta
...
dan juga sudah di tandai sebagai user tidak resmi`
...
if (chat.welcome && !itsMe && oneMem)  conn.sendMessage(from, { contextInfo, text: welcomeText },{quoted: m})
}
break
...
if (chat.welcome && !itsMe && oneMem ) conn.sendMessage(from, { contextInfo, text: byeText },{quoted: m})
}
break


case 'promote':{
const promoteText = (chat.sPromote || '@user Cie yang jadi a dmin eakk :v').replace('user', await conn.getName(sender))     
conn.sendMessage(from, { contextInfo, text: promoteText },{quoted: m})
 
}
break
            
case 'demote':{
const demoteText = (chat.sDemote || '@user Yahaha ga admin lagi awokawok :v').replace('user', await conn.getName(sender))      
conn.sendMessage(from, { contextInfo,text: demoteText },{quoted: m})
}
break
